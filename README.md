...
| 14 | `command_calibration.py` | **Tier‑2 step (2)** — command→contact calibration | delivered force is analytically predictable (`2·clip(kp·(target−x_contact),0,F_limit)`, held-out **R²=0.998, ~1% err**); the **force-cap proxy is badly wrong** (R²=−20, ~2.5× over-state); a learned correction adds nothing → **analytical wins** for command→force |
| 15 | `gp_core.py` + `gp_groundtruth.py` + `gp_bench.py` | **Tier‑2 step (3+4)** — integrated grasp‑and‑place **transfer** benchmark (adaptation‑data + sensing‑noise), held‑out target gripper config | 3 discriminating families (uniform / off‑CoM / placement), full task incl. release+withdraw+settle, 5 selectors on an identical candidate set. **At good sensing a task‑aware heuristic solves it with 0 target data (geo=1.00=oracle); learned & analytic match, add nothing → learning NOT justified.** As `σ_CoM`→55mm the **estimate‑trusting** methods collapse to ~0.62–0.69 while **estimate‑ignoring** ones stay 1.00; a trivial **source fixed‑grasp lookup also =1.00** → the learned "advantage" is just a **fixed per‑family grasp**, no world model justified. Checks pass: disjoint sets, **0/92** solver flips at `dt/2`, controlled‑place labels |
