...
- `port_identification.py` — **makes the certificate honest on hardware** by adding the second error source `ε_Y`. The **separation principle**: in free space the contact force vanishes, so exciting the endpoint identifies the robot port `Y_robot` *uncontaminated* by the contact law. (A) free-space ID recovers the **true** port — floating exact (2.6e-17), and on the Panda the naive rigid-body `J M⁻¹Jᵀ` misses the true port by **94%** because of the finger tendon, so you *identify* `Y`, not assume it. (B) from contact `H=(Y+C⁻¹)⁻¹` alone `(Y,C)` is a **gauge family** (any `Y'` fits); free-space `Y` makes `C` unique. (C) `ε_Y` scales ~linearly with sensor noise. (D) the **two-source certificate** `‖ΔH‖ ≤ (ε_Y+ε_C)/(m(m−(ε_Y+ε_C)))` — port-ID error and sim-to-real contact-law error add, and conditioning `m` sets the amplification.
